...
ID3D11Buffer* Renderer::m_MaterialBuffer{};//ライト設定(平行光源)
...
void Renderer::Init()
{
	...
	// デプスステンシルバッファ作成
	ID3D11Texture2D* depthStencile{};//深度ステンシルバッファ
	...
	hr = m_Device->CreateBuffer(&bufferDesc, NULL, &m_ProjectionBuffer);//プロジェクション行列
	m_DeviceContext->VSSetConstantBuffers(2, 1, &m_ProjectionBuffer);//頂点シェーダーにセット
	if (FAILED(hr)) return;
	...
	//鏡用レンダーテクスチャの初期化
	MirrorRenderTexture::Init(m_Device);
...
void Renderer::Begin()
{
	...
	m_DeviceContext->ClearRenderTargetView(m_RenderTargetView, clearColor);//レンダーターゲットのクリア
	m_DeviceContext->ClearDepthStencilView(m_DepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);//深度ステンシルのクリア
...
//レンダーターゲットビュー取得
//=======================================
ID3D11RenderTargetView* Renderer::GetRenderTargetView(void)
{
